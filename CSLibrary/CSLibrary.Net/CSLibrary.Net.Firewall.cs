...
namespace CSLibrary.Net
{
    class Security
    {
        ...
        public FW_ERROR_CODE Initialize()
        {
            ...
            typFwMgr = Type.GetTypeFromProgID("HNetCfg.FwMgr");
        ...
        public FW_ERROR_CODE AddApplication(string strProcessImageFileName, string strRegisterName)
        {
            ...
            if (bAppEnable == false)
            {
                ...
                Type typeFwApp = Type.GetTypeFromProgID(PROGID_AUTHORIZED_APPLICATION);
        ...
        public FW_ERROR_CODE AddPort(int nPortNumber, NET_FW_IP_PROTOCOL_ ipProtocol, string strRegisterName)
        {
            ...
            if (bEnablePort == false)
            {
                ...
                INetFwOpenPort FWOpenPort = (INetFwOpenPort)Activator.CreateInstance(Type.GetTypeFromProgID("HNetCfg.FwOpenPort"));
                if (FWOpenPort == null)
    ...
    {
        private static Security m_fwsecutiry = null;
        /// <summary>
        /// Add firewall exception in windows
        /// </summary>
        /// <param name="strProcessImageFileName">file path of the application</param>
        /// <param name="strRegisterName">Name of the firewall exception</param>
        /// <returns></returns>
        public static bool AddAplication(string strProcessImageFileName, string strRegisterName)
        {
            try
            {
                if (m_fwsecutiry == null)
                {
                    m_fwsecutiry = new Security();
                    m_fwsecutiry.Initialize();
                }
                if (m_fwsecutiry.AddApplication(strProcessImageFileName, strRegisterName) == Security.FW_ERROR_CODE.FW_NOERROR)
                {
                    return true;
                }
            }
        ...
        /// <summary>
        /// Remove firewall exception in windows
        /// </summary>
        /// <param name="strRegisterName">Name of the firewall exception</param>
        /// <returns></returns>
        public static bool RemoveApplication(string strRegisterName)
        {
            try
            {
                if (m_fwsecutiry == null)
                ...
                if (m_fwsecutiry.RemoveApplication(strRegisterName) == Security.FW_ERROR_CODE.FW_NOERROR)
                {
                    return true;
                }
            }
