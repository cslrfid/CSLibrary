...
namespace CSLibrary.Packet
{
    ...
    class PacketData
    {
        ...
        public enum ErrCode : ushort
        {
            VerifyFailed = 0x01,// - Read after write verify failed.
            TagCmdFailed = 0x02,// - Problem transmitting tag command.
            CRCWriteFailed = 0x03,// - CRC error on tag response to a write.
            CRCReadFailed = 0x04,// - CRC error on the read packet when verifying the write.
            MaxTryFailed = 0x05,// - Maximum retry's on the write exceeded.
            ReadTimeout = 0x06,// - Failed waiting for read data from tag, possible timeout.
            TagHandleFailed = 0x07,// - Failure requesting a new tag handle.
            ResponseTimeout = 0x0A,// - Error waiting for tag response, possible timeout.
            CRCKillFailed = 0x0B,// - CRC error on tag response to a kill.
            TranKillFailed = 0x0C,// - Problem transmitting 2nd half of tag kill.
            KillHandleFailed = 0x0D,// - Tag responded with an invalid handle on first kill command.
        }

        //[Serializable]
        public class PacketWrapper// : System.Xml.Serialization.IXmlSerializable//ISerializable
        {
...
//					_commandPacketNumber= (int?)information.GetValue("commandPacketNumber", typeof(int?));
        ...
        public class PacketBase
        {
            ...
            public static string GetTagAccessTypeName(int access)
            {
                switch ((ISO_18000_6C_TagAccessType)access)
                {
                    ...
                    case ISO_18000_6C_TagAccessType.TAG_ACCESS_ERASE:
                        return "erase";
            ...
            */
            ...
            public static string GetStatusName(uint value)
            {
                if (Enum.IsDefined(typeof(commandstatus), (int)value))
                {
                    commandstatus cs = (commandstatus)value;
                    switch (cs)
                    {
            ...
            public override string ToString()
            {
                ...
                foreach (System.Reflection.FieldInfo info in this.GetType().GetFields())
                {
                    if (info.FieldType.IsArray)
                    {
                        ...
                        sb.AppendFormat(null, "{0}: (array) ", info.Name);
                    }
                    else
                    {
                        sb.AppendFormat(null, "{0}: {1} ", info.Name, info.GetValue(this).ToString());
                    }
                }
                return sb.ToString();
            }
        ...
        public class cmd_end : PacketBase
        {
            ...
            public INT32U ms_ctr;						// current millisecond timer/counter
            public INT32U status;						// command status indicator - usually macerrno
        }
        ...
        public class inventory : PacketBase
        {
            ...
            public INT32U ms_ctr;					// current millisecond timer/counter
            public INT16U rssi;					    // Narrow-Band Receive Signal Strength Indicator - backscattered tab signal amplitude.
            public INT16U ana_ctrl1;                // 
            public INT32U res0 = 0;
        ...
        //[Serializable]
        [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Auto)]
        public class Iso18k6c_acc : PacketBase
        {
            // Common Preamble
            public readonly INT8U pkt_ver = 1;
            public INT8U flags;
            ...
            public INT16U pkt_len;
            public readonly INT16U res0;
            // End of Common Preamble
            public INT32U ms_ctr;				// current millisecond timer/counter
            public INT8U command;				// ISO 18000-6C access command (0xC2 ?Read, 0xC3 ?Write, 0xC4 ?Kill, 0xC5 ?Lock, 0xC6 ?Erase)
            public INT8U error_code;			// If the tag backscattered an error (i.e., the tag error flag is set), 
            // this value is the error code that the tag backscattered.  Values are:
            //	0x00 ?general error (catch-all for errors not covered by codes)
            //	0x03 ?specified memory location does not exist of the PC value is not supported by the tag
            //	0x04 ?specified memory location is locked and/or permalocked and is not writeable
        ...
        * ********/
        ...
        /// <param name="packet">output parameter to hold the message header.</param>
        ...
        public static PacketType ParsePacket(byte[] buffer, out PacketBase packet, out string errorMessage)
        {
            ...
            if (buffer == null)
            {
                ...
                errorMessage = "buffer Attempted to parse an empty packet buffer.";
                return PacketType.U_N_D_F_I_N_E_D;
            }
            ...
            if (buffer.Length < PREAMBLE_SIZE)
            {
                ...
                errorMessage = "PacketDataTooSmall";
                return PacketType.U_N_D_F_I_N_E_D;
            }
            ...
            if (rslt == PacketType.U_N_D_F_I_N_E_D)
            {
                ...
                errorMessage = "UnknownPacketType";
                return PacketType.U_N_D_F_I_N_E_D;
            }
            ...
            try
            {
                CSLibrary.Utility.Counter timer = new CSLibrary.Utility.Counter();
                timer.Start();
                Parse_Fields(packet, buffer, 0, FullPacketBytes(buffer, 0));
                timer.Stop();
                Debug.WriteLine(string.Format("Parse_Fields time = {0}", timer.Duration));
    ...
    class RegisterData
    {
        ...
        public class QueryConfigRegister
        {
            ...
            public CSLibrary.Constants.SessionTarget Target
            {
                get
                {
                    int val = _value[target];

                    return Enum.IsDefined(typeof(CSLibrary.Constants.SessionTarget), val) ?
                        (CSLibrary.Constants.SessionTarget)val : CSLibrary.Constants.SessionTarget.UNKNOWN;
                }
            ...
            public CSLibrary.Constants.SessionTarget Session
            {
                get
                {
                    ...
                    return Enum.IsDefined(typeof(CSLibrary.Constants.SessionTarget), val) ?
                        (CSLibrary.Constants.SessionTarget)val :
                        CSLibrary.Constants.SessionTarget.UNKNOWN;
                }
